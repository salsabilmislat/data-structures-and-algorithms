...
const longestString = (arr) => {
  ...
  let longestString = arr.map(item => item.length).indexOf(Math.max(...arr.map(item => item.length))
  );
...
const onlyOddChars = (str) => {
  ...
  return str.split('').reduce((accumulator, newValue, idx) => (idx % 2 !== 0) ? accumulator += newValue : accumulator, '');
};
