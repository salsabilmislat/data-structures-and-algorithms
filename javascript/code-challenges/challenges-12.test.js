...
const validateEmail = (email) => {
  // Solution code here...
  let emailChecked =/^\w+\.?\w+@\w+.(com|net|org)$/;
...
const validatePhoneNumber = (phoneNumber) => {
  // Solution code here...
  let numOfPhone = /^(\((\d{3})\)[\s]?|(\d{3})[\s-]?)(\d{3})[\s-]?(\d{4})$/;
  
  return numOfPhone.test(phoneNumber);
};
